...
export const AgreementDoc: React.FC<Props> = ({ data, showPadHeaderSpace = true }) => {
  ...
  // Calculate charge display based on method
  let serviceChargeDisplay = `${data.calculatedServiceCharge || 0}/-`;
  let totalChargeDisplay = `${((Number(data.calculatedServiceCharge) || 0) * studentNum).toLocaleString()} BDT`;
  let paymentTypeDisplay = 'বাৎসরিক';

  if (data.chargeMethod === 'transaction_percentage') {
    serviceChargeDisplay = `${data.transactionPercentage || '1.5'}% (প্রতি ট্রাঞ্জেকশন)`;
    totalChargeDisplay = `${data.transactionPercentage || '1.5'}% প্রতি ট্রাঞ্জেকশন ফি কর্তন`;
    paymentTypeDisplay = 'পার ট্রাঞ্জেকশন (%)';
  } else if (data.chargeMethod === 'monthly_charge') {
    const monthlyAmt = Number(data.monthlyChargeAmount) || Number(data.calculatedServiceCharge) || 0;
    serviceChargeDisplay = `${monthlyAmt.toLocaleString()}/- (প্রতি মাস)`;
    totalChargeDisplay = `${(monthlyAmt * 12).toLocaleString()} BDT (বাৎসরিক)`;
    paymentTypeDisplay = 'মাসিক';
  }

  if (data.enablePerTransactionRate && data.perTransactionRateAmount) {
    serviceChargeDisplay += ` + ${data.perTransactionRateAmount}/- (প্রতি ট্রাঞ্জেকশন ফিক্সড)`;
  }

  return (
    <div className="bg-white text-black font-serif text-sm leading-relaxed p-8 max-w-[850px] mx-auto printable-document legal-page-document">
      {/* Government Stamp Paper Space */}
      {showPadHeaderSpace && (
        <div className="h-[3.25in] border-2 border-dashed border-black rounded-md mb-6 flex flex-col items-center justify-center text-black text-xs bg-white pad-space-box">
          <span className="font-sans font-bold text-black mb-1 pad-space-label no-print">সরকারি স্ট্যাম্প / প্যাডের জন্য নির্ধারিত স্থান</span>
          <span className="text-[11px] text-black pad-space-label no-print">(প্রিন্ট করার সময় এই ৩.২৫ ইঞ্চি ফাঁকা জায়গায় সরকারি স্ট্যাম্প বসবে)</span>
        </div>
      )}

      {/* Header */}
      <div className="text-center mb-6 pb-4">
        <h1 className="text-2xl font-bold tracking-tight text-black mb-1 font-sans">একাডেমির সেবা চুক্তি</h1>
        <p className="text-base font-semibold text-black font-sans">[শিক্ষা প্রতিষ্ঠান ব্যবস্থাপনা সফটওয়্যার]</p>
      </div>

      {/* Contract Dates */}
      <div className="grid grid-cols-2 gap-4 bg-white p-3 rounded border border-black mb-6 font-sans text-xs">
        <div>
          <span className="font-semibold text-black">চুক্তির তারিখঃ </span>
          <span className="font-bold text-black">{data.date || '০৮/০৮/২০২৬'}</span>
        </div>
        <div>
          <span className="font-semibold text-black">চুক্তির মেয়াদঃ </span>
          <span className="font-bold text-black">{expiryDate}</span>
        </div>
      </div>
      ...
      <div className="space-y-6 mb-6">
        ...
        <div className="border border-black rounded overflow-hidden">
          <div className="bg-white text-black font-bold px-4 py-2 font-sans text-xs">
            ১ম পক্ষ ( অটোমেট আইটি লিমিটেড)
          </div>
          <div className="p-4 grid grid-cols-2 gap-x-6 gap-y-2 text-xs font-sans text-black">
            <div><span className="font-semibold text-black">নামঃ</span> আতিকুর রহমান</div>
            <div><span className="font-semibold text-black">পদবীঃ</span> ম্যানেজার, বিজনেস অপারেশন্স</div>
            <div><span className="font-semibold text-black">মোবাইল নম্বরঃ</span> ০১৬২৯১৮০৩৪৯</div>
            <div><span className="font-semibold text-black">ইমেলঃ</span> automateitbd@gmail.com</div>
            <div className="col-span-2"><span className="font-semibold text-black">অফিস ঠিকানাঃ</span> হাউজ- ৩৪/এ, রোড- ১৩/২, বল্ক- কে, সাউথ বনশ্রী, ঢাকা-১২১৯</div>
          </div>
        </div>
        ...
        <div className="border border-black rounded overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-x-6 gap-y-2 text-xs font-sans text-black">
            <div className="col-span-2"><span className="font-semibold text-black">প্রতিষ্ঠানের নামঃ</span> <span className="font-bold text-black">{data.instituteName || '—'}</span></div>
            <div><span className="font-semibold text-black">কর্তৃপক্ষের নামঃ</span> {data.headName || '—'}</div>
            <div><span className="font-semibold text-black">পদবীঃ</span> {data.designation || '—'}</div>
            <div><span className="font-semibold text-black">মোবাইল নম্বরঃ</span> {data.headMobile || '—'}</div>
            <div><span className="font-semibold text-black">Telephone:</span> —</div>
            <div className="col-span-2"><span className="font-semibold text-black">ইমেলঃ</span> {data.instituteEmail || '—'}</div>
            <div><span className="font-semibold text-black">পরিচালনা কমিটির সভাপতিঃ</span> {data.chairmanName || '—'}</div>
            <div><span className="font-semibold text-black">মোবাইল নম্বরঃ</span> {data.chairmanMobile || '—'}</div>
            <div><span className="font-semibold text-black">ICT ইন-চার্জঃ</span> {data.ictInchargeName || '—'}</div>
            <div><span className="font-semibold text-black">মোবাইল নম্বরঃ</span> {data.ictInchargeMobile || '—'}</div>
            <div className="col-span-2"><span className="font-semibold text-black">ঠিকানাঃ</span> {data.instituteAddress || '—'}</div>
            <div><span className="font-semibold text-black">উপজেলা/থানাঃ</span> {data.upazilaThana || '—'}</div>
            <div><span className="font-semibold text-black">জেলাঃ</span> {data.district || '—'}</div>
            <div><span className="font-semibold text-black">বিভাগঃ</span> {data.division || '—'}</div>
            <div><span className="font-semibold text-black">প্রতিষ্ঠানের ধরণঃ</span> {data.instituteType || '—'}</div>
            <div><span className="font-semibold text-black">শিক্ষা বোর্ডঃ</span> {data.educationBoard || '—'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="border border-black rounded overflow-hidden mb-6">
        ...
        <div className="p-4 space-y-3 text-xs font-sans text-black">
          <div>
            <span className="font-semibold text-black block mb-1">মডিউল লিস্টঃ</span>
            <div className="bg-white p-3 rounded border border-black text-xs flex flex-wrap gap-1.5">
              {allModules.map((m, idx) => (
                <span key={idx} className="bg-white px-2 py-1 rounded border border-black font-medium text-black">
                  {m}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div><span className="font-semibold text-black">শিক্ষার্থীর সংখ্যাঃ</span> {data.studentCount || '৩০০'}</div>
            <div><span className="font-semibold text-black">শিক্ষক ও কর্মচারীদের সংখ্যাঃ</span> {data.teacherCount || '—'}</div>
          </div>

          <div className="text-black italic text-[11px] bg-white p-2.5 rounded border border-black">
            নোটঃ নূন্যতম শিক্ষার্থী সংখ্যা ৩০০জন হতে হবে, এর কম শিক্ষার্থী হলেও ৩০০জন শিক্ষার্থী অনুপাতে সার্ভিস চার্জ প্রযোজ্য হবে।
          </div>

          <div className="grid grid-cols-2 gap-3 pt-2 font-medium text-black">
            <div><span className="font-semibold text-black">পেমেন্ট টাইপঃ</span> {paymentTypeDisplay}</div>
            <div><span className="font-semibold text-black">সার্ভিস চার্জঃ</span> {serviceChargeDisplay}</div>
            <div className="col-span-2"><span className="font-semibold text-black">টোটাল সার্ভিস চার্জঃ</span> {totalChargeDisplay}</div>
          </div>

          <div className="grid grid-cols-2 gap-3 pt-2 text-xs text-black">
            <div>
              <span className="font-semibold text-black">ওয়েবসাইট সার্ভিস চার্জ, হোস্টিং এবং বাৎসরিক রিনিউ চার্জ:</span> {data.websiteRenewalFee ? `${data.websiteRenewalFee}/-` : '—'}
            </div>
            <div><span className="font-semibold text-black">রেজিস্ট্রেশন চার্জঃ</span> ১০০০/-</div>
          </div>
        </div>
      </div>

      {/* Recital */}
      <p className="text-xs text-justify leading-relaxed mb-4 font-sans text-black">
      ...
      <div className="my-4 pt-2 pb-2 bg-white px-4 rounded font-sans text-xs flex justify-between items-center text-black border border-black">
        <div>
          <span className="font-bold text-black">১ম পক্ষ (অটোমেট আইটি) স্বাক্ষর:</span> ______________________
        </div>
        <div>
          <span className="font-bold text-black">২য় পক্ষ ({data.instituteName || 'শিক্ষা প্রতিষ্ঠান'}) স্বাক্ষর ও সীল:</span> ______________________
        </div>
        <span className="text-[10px] text-black italic">[ পেজ স্বাক্ষর ]</span>
      </div>

      {/* Terms and Conditions */}
      <div className="mb-8 text-black">
        <h3 className="font-bold text-sm pb-1 mb-3 font-sans text-black">শর্তাবলী</h3>
        <ol className="list-decimal pl-5 space-y-2 text-xs text-justify leading-relaxed font-sans text-black">
          ...
          <li>যেহেতু ২য় পক্ষের গুরুত্বপূর্ণ ডাটা সমূহ ১ম পক্ষের নিয়ন্ত্রণাধীন অনলাইন সার্ভারে সংরক্ষিত থাকবে।  সেহেতু ২য় পক্ষের গুরুত্বপূর্ণ সকল ডাটার নিরাপত্তা ও ব্যাকআপ ১ম পক্ষ নিখুঁতভাবে প্রদান করবে। ২য় পক্ষ সফটওয়্যার এর এডমিন ড্যাশবোর্ড হতে প্রতিনিয়ত ডাটা ব্যাকআপ নিয়ে এক্সেল ফরমেটে নিতে পারবে। যা ২য় পক্ষ সম্পূর্ন ফ্রিতে উপভোগ করবে এবং ২য় পক্ষের প্রয়োজন মাফিক সংরক্ষণ করতে পারবে। উল্লেখ্য,  কোন রকম আবেদন ছাড়া ৩ মাসের বেশি সময় বিল বকেয়া অবস্থায় আইডি বন্ধ থাকলে ১ম পক্ষের সার্ভার হতে সমস্ত ডাটা মুছে ফেলা হবে।  সে ক্ষেত্রে ২য় পক্ষের কোনো প্রকার দাবি অভিযোগ ও আপত্তি গ্রহণযোগ্য হবে না।</li>
      ...
      <p className="text-xs text-justify leading-relaxed mb-8 font-sans text-black">
        এমতাবস্থায়, আমরা উভয় পক্ষ চুক্তিপত্রের উভয় পৃষ্ঠার সকল তথ্য ও শর্ত সমূহের মর্ম উপলব্ধি করে সুস্থ্য মস্তিস্কে, স্ব-জ্ঞানে, অন্যের বিনা প্ররোচনায় নিম্নোক্ত স্বাক্ষীগণের উপস্থিতিতে অদ্য <span className="font-bold text-black">{data.date || '০৮/০৮/২০২৬'}</span>ইং তারিখ অত্র চুক্তিনামায় স্বাক্ষর প্রদান করছি।
      </p>

      {/* Signature Section */}
      <div className="grid grid-cols-2 gap-8 pt-8 font-sans text-xs text-black">
        <div>
          <div className="font-bold mb-12 text-black">অটোমেট আইটি লিমিটেড এর পক্ষে</div>
          <div className="pt-2">
            <p className="font-bold text-black">আতিকুর রহমান</p>
            <p className="text-black">ম্যানেজার, বিজনেস অপারেশন্স</p>
            <p className="mt-6 text-[11px] text-black">স্বাক্ষী- ………………………………………………………………..</p>
          </div>
        </div>

        <div>
          <div className="font-bold mb-12 text-black">“{data.instituteName || 'শিক্ষা প্রতিষ্ঠান'}” এর পক্ষে</div>
          <div className="pt-2">
            <p className="font-bold text-black">নামঃ {data.headName || '—'}</p>
            <p className="text-black">পদবীঃ {data.designation || '—'}</p>
            <p className="mt-6 text-[11px] text-black">স্বাক্ষী- ………………………………………………………………..</p>
          </div>
        </div>
      </div>
