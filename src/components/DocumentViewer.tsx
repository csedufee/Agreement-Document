...
import { 
  FileText, Download, Layers, ArrowLeft, RefreshCw, 
  CheckCircle2, FolderKanban
} from 'lucide-react';
import { DocumentType, InstituteAgreementData } from '../types';
import { DOCUMENT_LIST, DOCUMENT_CATEGORIES } from '../data/modules';
...
import { BoardDeclarationDoc } from './DocumentTemplates/BoardDeclarationDoc';
import { DomainForwardingDoc } from './DocumentTemplates/DomainForwardingDoc';
import { SPGApprovalDoc } from './DocumentTemplates/SPGApprovalDoc';
import { SPGVendorChangeDoc } from './DocumentTemplates/SPGVendorChangeDoc';
import { SPGSplitAccountDoc } from './DocumentTemplates/SPGSplitAccountDoc';
import { SPGSoftwareChargeDoc } from './DocumentTemplates/SPGSoftwareChargeDoc';
import { WebsiteWorkOrderDoc } from './DocumentTemplates/WebsiteWorkOrderDoc';
...
export const DocumentViewer: React.FC<Props> = ({ data, onBackToEdit }) => {
  // Filter active documents list based on user's selected documents
  const activeDocumentList = (data.selectedDocuments && data.selectedDocuments.length > 0)
    ? DOCUMENT_LIST.filter(d => data.selectedDocuments!.includes(d.id))
    : DOCUMENT_LIST;

  const initialDoc = activeDocumentList[0]?.id || 'agreement';
  const [activeTab, setActiveTab] = useState<DocumentType>(initialDoc);
  const [downloading, setDownloading] = useState(false);
  const [progressMsg, setProgressMsg] = useState<string | null>(null);

  useEffect(() => {
    if (activeDocumentList.length > 0 && !activeDocumentList.some(d => d.id === activeTab)) {
      setActiveTab(activeDocumentList[0].id);
    }
  ...
  const docRefs = useRef<Record<DocumentType, HTMLDivElement | null>>({
    ...
    declaration: null,
    board_declaration: null,
    domain_forwarding: null,
    spg_approval: null,
    spg_vendor_change: null,
    spg_split_account: null,
    spg_software_charge: null,
    website_work_order: null
  });
  ...
  const handleDownloadZipPackage = async () => {
    setDownloading(true);
    setProgressMsg(`Generating ${activeDocumentList.length} PDF files...`);
  ...
  const renderActiveDocument = (type: DocumentType) => {
    switch (type) {
      ...
      case 'board_declaration':
        return <BoardDeclarationDoc data={data} showPadHeaderSpace={true} />;
      case 'domain_forwarding':
        return <DomainForwardingDoc data={data} showPadHeaderSpace={true} />;
      case 'spg_approval':
        return <SPGApprovalDoc data={data} showPadHeaderSpace={true} />;
      case 'spg_vendor_change':
        return <SPGVendorChangeDoc data={data} showPadHeaderSpace={true} />;
      case 'spg_split_account':
        return <SPGSplitAccountDoc data={data} showPadHeaderSpace={true} />;
      case 'spg_software_charge':
        return <SPGSoftwareChargeDoc data={data} showPadHeaderSpace={true} />;
      case 'website_work_order':
        return <WebsiteWorkOrderDoc data={data} showPadHeaderSpace={true} />;
      default:
        return <AgreementDoc data={data} showPadHeaderSpace={true} />;
    }
  };
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-4 sm:p-6 shadow-xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-xs text-slate-300">
            {activeDocumentList.length} selected official documents prepared below.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          {onBackToEdit && (
            <button
              ...
              className="bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-3.5 py-2.5 rounded-xl border border-slate-700 flex items-center gap-1.5 transition-colors"
            >
          ...
          <div className="text-xs font-medium px-3.5 py-2.5 rounded-xl border bg-slate-800/80 border-slate-700/80 text-slate-300 flex items-center gap-1.5 shadow-inner">
          ...
          <button
            ...
            className="bg-slate-800 hover:bg-slate-700 text-white font-medium text-xs px-3.5 py-2.5 rounded-xl border border-slate-700 flex items-center gap-1.5 transition-colors"
          >
          ...
          >
            ...
            <span>Download All ({activeDocumentList.length}) PDFs (ZIP)</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-2 shadow-sm border border-slate-200 no-print overflow-x-auto">
        <div className="flex gap-1.5 min-w-max p-1">
          {activeDocumentList.map((doc, idx) => {
